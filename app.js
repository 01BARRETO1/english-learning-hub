...
const sectionIcons = {
    'Grammar': 'fa-spell-check', 'Vocabulary': 'fa-book',
    'Listening': 'fa-headphones', 'Reading': 'fa-book-open',
    'Writing': 'fa-pen-fancy', 'Exam': 'fa-clipboard-check'
};
const sectionNames = {
    'Grammar': 'Gramática', 'Vocabulary': 'Vocabulario',
    'Listening': 'Listening', 'Reading': 'Reading',
    'Writing': 'Writing', 'Exam': 'Examen'
};
const levelColors = {
...
document.addEventListener('DOMContentLoaded', () => {
    const levelSelect = document.getElementById('level-select');
    levelSelect?.addEventListener('change', (e) => changeLevel(e.target.value));
...
function changeLevel(level) {
    currentLevel = level; currentSection = ''; currentLessonIndex = 0;
    if (typeof courseData !== 'undefined') {
        updateGlobalProgress(level, courseData);
        updateStatsPanel(courseData);
    }
...
function renderLesson() {
    ...
    if (lesson.vocabList && lesson.vocabList.length > 0) {
        html += `
            <div class="lesson-card">
                <h3><i class="fas fa-book"></i> Vocabulario</h3>
                <table class="vocab-table">
                    <thead><tr><th>Palabra</th><th>Tipo</th><th>Significado</th><th>Ejemplo</th></tr></thead>
                    <tbody>
                        ${lesson.vocabList.map(v => `
                            <tr>
                                <td class="word-en">${escapeHtml(v.word)} <span class="word-type">(${v.type})</span></td>
    ...
    if (lesson.writingPrompt) {
        html += `
            <div class="lesson-card">
                ...
                <textarea class="writing-textarea" id="writing-area" placeholder="Escribe tu respuesta aquí..."></textarea>
                <div class="writing-word-count" id="word-count">0 palabras (mínimo: ${lesson.wordCount || 50})</div>
                <button class="btn-primary mt-20" onclick="checkWriting()"><i class="fas fa-check"></i> Comprobar</button>
            </div>
        `;
        setTimeout(() => {
            const textarea = document.getElementById('writing-area');
    ...
    if (lesson.exercises && lesson.exercises.length > 0) {
        html += `
            <button class="btn-primary" onclick='startQuiz(${JSON.stringify(lesson.exercises).replace(/'/g, "\\'")}, "Práctica: ${lesson.title.replace(/'/g, "\\'")}", "${lessonId}", false)'>
                <i class="fas fa-play"></i> Practicar (${lesson.exercises.length} preguntas)
            </button>
        `;
    }
    ...
    if (lesson.exam && lesson.exam.length > 0) {
        html += `
            <button class="btn-success" onclick='startQuiz(${JSON.stringify(lesson.exam).replace(/'/g, "\\'")}, "Examen: ${lesson.title.replace(/'/g, "\\'")}", "${lessonId}", true)'>
                <i class="fas fa-clipboard-check"></i> Hacer Examen (${lesson.exam.length} preguntas)
            </button>
        `;
    }
    ...
    if (!isComplete) {
        html += `<button class="btn-secondary" onclick="markLessonComplete('${lessonId}'); renderLesson();"><i class="fas fa-check-circle"></i> Marcar como completada</button>`;
    } else {
        html += `<span style="color:var(--success); font-weight:600;"><i class="fas fa-check-circle"></i> Completada</span>`;
    }
    ...
    html += `
        <div class="lesson-navigation">
            <button class="nav-prev" ${currentLessonIndex === 0 ? 'disabled' : ''} onclick="prevLesson()"><i class="fas fa-arrow-left"></i> Anterior</button>
            <span class="lesson-counter">${currentLessonIndex + 1} / ${lessons.length}</span>
            <button class="nav-next" ${currentLessonIndex === lessons.length - 1 ? 'disabled' : ''} onclick="nextLesson()">Siguiente <i class="fas fa-arrow-right"></i></button>
        </div>
    `;
...
function checkWriting() {
    ...
    showToast('¡Buen trabajo! Has escrito ' + words + ' palabras.', 'success');
    const lessonId = `${currentLevel}-${currentSection}-${currentLessonIndex}`;
    markLessonComplete(lessonId);
}

function prevLesson() {
    if (currentLessonIndex > 0) { currentLessonIndex--; renderLesson(); }
}

function nextLesson() {
    const lessons = courseData[currentLevel][currentSection];
    if (currentLessonIndex < lessons.length - 1) { currentLessonIndex++; renderLesson(); }
}
